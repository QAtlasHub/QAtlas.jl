...
```julia
verify(TFIM(; J = 1.0, h = h), XXStructureFactor(), Infinite(); route = :delegation_invariant, independent = QAtlas.fetch(TFIM(; J = 1.0, h = h), XXStructureFactor(), OBC(N_proxy); beta = β, q = q), agree_within = 0.0, at = ["N_proxy=$(N_proxy)"], refs = ["Infinite() proxy returns exactly the OBC(N_proxy) value by construction (pass-through identity, == compared)"], fetch_kw = (; beta = β, q = q, N_proxy = N_proxy))
```
