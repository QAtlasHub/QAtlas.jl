...
# ── Per-model API-docs injection ────────────────────────────────────────────
# `fetch(::Model, …)` only exists as per-model `@register`+method pairs, so its
# docstrings belong on each model's own page rather than in one global index.
# For every page we map its models to their source directories (by scanning
# src/models for the `struct … <: AbstractQAtlasModel` definitions), glob those
# directories, and emit an `@autodocs` scoped to the files between ATLAS:DOCS
# markers — rendering the model struct, its `fetch` methods, and exported
# helpers (Private=false skips internal `_*`).
const _ATLAS_DOCS_BEGIN = "<!-- ATLAS:DOCS:START -- auto-generated by docs/atlas/generate.jl. Do not edit by hand; edits between these markers are overwritten on next regen. -->"
const _ATLAS_DOCS_END = "<!-- ATLAS:DOCS:END -->"
# Model name -> source directory, by scanning src/models for its struct def.
# generate.jl is QAtlas-free by design (a static source view), so we parse the
# source tree rather than introspect `methods(fetch)`.
const _MODEL_SRC_DIRS = let
    d = Dict{String,String}()
    for (root, _, files) in walkdir(joinpath(ROOT, "src", "models"))
        for f in files
            endswith(f, ".jl") || continue
            for mm in eachmatch(
                r"^struct\s+([A-Z][A-Za-z0-9_]*)\s*(?:\{[^}]*\})?\s*<:\s*AbstractQAtlasModel"m,
                read(joinpath(root, f), String),
            )
                d[mm.captures[1]] = root
            end
        end
    end
    d
...
function _model_doc_pages(model_names)
    srcroot = joinpath(ROOT, "src")
    files = String[]
    for mn in model_names
        haskey(_MODEL_SRC_DIRS, mn) || continue
        for f in readdir(_MODEL_SRC_DIRS[mn])
            endswith(f, ".jl") && push!(files, joinpath(_MODEL_SRC_DIRS[mn], f))
        end
    end
    return sort(unique([_posix(relpath(f, srcroot)) for f in files]))
end

function render_model_docs_section(model_names)
    pages = _model_doc_pages(model_names)
    io = IOBuffer()
    P = (xs...) -> println(io, xs...)
    P(_ATLAS_DOCS_BEGIN)
    P()
    ...
    P()
    if isempty(pages)
        P("_No `fetch` source files resolved for this page._")
        P()
    else
        subj = length(model_names) == 1 ? "this model" : "these models"
        P(
            "Every `fetch(::Model, …)` method registered for ",
            subj,
            " — together with the model struct(s) and exported helpers — ",
            "generated directly from the source (in lock-step with `@register`):",
        )
        P()
        P("```@autodocs")
        P("Modules = [QAtlas]")
        P("Pages = [", join(("\"$p\"" for p in pages), ", "), "]")
        P("Private = false")
        P("Order = [:type, :function]")
        P("```")
        P()
    end
    P(_ATLAS_DOCS_END)
...
function inject_model_docs_section!(page_rel, model_names)
    p = joinpath(ROOT, page_rel)
    if !isfile(p)
        @warn "ATLAS docs-inject skip: page not found" page = page_rel
        return nothing
    end
    content = read(p, String)
    section = render_model_docs_section(model_names)
    b = findfirst(_ATLAS_DOCS_BEGIN, content)
    e = findfirst(_ATLAS_DOCS_END, content)
    if (b === nothing) != (e === nothing)
        @warn "ATLAS docs-inject skip: page has only one of START/END marker" page =
            page_rel
        return nothing
    end
    if b !== nothing && e !== nothing && first(e) <= last(b)
        @warn "ATLAS docs-inject skip: START/END markers in wrong order" page = page_rel
        return nothing
    end
    if b !== nothing && e !== nothing
        new = content[1:(first(b) - 1)] * section * content[(last(e) + 1):end]
    ...
    if new != content
        write(p, new)
        println("  injected API docs into ", page_rel)
    end
end

for page in sort(collect(keys(_PAGE_TO_MODELS)))
    inject_hub_section!(page, _PAGE_TO_MODELS[page])
    inject_model_docs_section!(page, _PAGE_TO_MODELS[page])
end
