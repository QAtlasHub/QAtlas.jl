...
function _xxz_szz_term(Q_val::Real, ω::Real, qe::Real, k::Real, kp::Real, K_val::Real, ε::Real, κ::Real, Δ::Real, σ::Real)
    sinQ = sin(Q_val)
    if abs(sinQ) < 1e-12
        return 0.0
    end
    ...
    dn_arg = ((1.0 + cos(Q_val)) / abs(sinQ)) * sqrt((ω^2 - κ^2 * ω0^2 + B) / (ω^2 + κ^2 * ω0^2 - B))
...
function fetch(
    ...
    model.Δ > 1.0 || throw(DomainError(model.Δ, "XXZ1D exact 2-spinon longitudinal DSF is only defined in the massive regime Δ > 1."))
    
    if method !== :exact_2spinon
        throw(ArgumentError("XXZ1D ZZStructureFactor Infinite: unknown method :$method; supported is :exact_2spinon."))
    end
    ...
    term_plus = 0.0
    if in_C_plus
        term_plus = ((1.0 - κ) / (1.0 + κ)) * _xxz_szz_term(Q_norm, ω_unit, qe, k, kp, K_val, ε, κ, model.Δ, 1.0)
    end
    
    term_minus = 0.0
    if in_C_minus
        term_minus = _xxz_szz_term(Q_reflected, ω_unit, qe, k, kp, K_val, ε, κ, model.Δ, -1.0)
    end
