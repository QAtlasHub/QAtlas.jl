...
@register(
    ...
    cost=:polynomial,
    status=:approx,
    valid_domain="thermodynamic limit; the derivative is taken at the registered beta",
    error_order="central finite difference with h = 1e-6 -- a NUMERICAL DERIVATIVE of the exact free energy, so the error is truncation + round-off, not a physics approximation",
    reliability=:high,
    tested_in="test/models/classical/test_six_vertex.jl",
    references=["Lieb1967a", "Lieb1967b", "Sutherland1967", "Baxter1982"],
