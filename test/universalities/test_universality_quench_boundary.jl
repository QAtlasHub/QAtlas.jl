...
@testset "Universality :: Quench Dynamics & Boundary Entropies" begin
    ...
    @testset "Infinite BC global quench" begin
        ...
        t1 = 1.0
        S1 = QAtlas.fetch(model, VonNeumannEntropy(:quench), Infinite(); ℓ=ℓ, t=t1, v=v, beta_eff=beta_eff)
        @test S1 ≈ (2 * v * t1) * π * c / (6 * beta_eff) atol=1e-12

        # t = 5.0 (t > ℓ/(2v) = 3.333...) -> saturation
        # S = ℓ * pi * c / (6 * beta_eff) = 10 * pi * 0.5 / 12 = 5/12 * pi
        t2 = 5.0
        S2 = QAtlas.fetch(model, VonNeumannEntropy(:quench), Infinite(); ℓ=ℓ, t=t2, v=v, beta_eff=beta_eff)
    ...
    @testset "OBC global quench with boundary state" begin
        ...
        t1 = 1.0
        S1 = QAtlas.fetch(model, VonNeumannEntropy(:quench), OBC(); ℓ=ℓ, t=t1, v=v, beta_eff=beta_eff)
        @test S1 ≈ (v * t1) * π * c / (12 * beta_eff) atol=1e-12

        # t = 4.0 (t > ℓ/v = 2.5) -> saturation
        t2 = 4.0
        S2 = QAtlas.fetch(model, VonNeumannEntropy(:quench), OBC(); ℓ=ℓ, t=t2, v=v, beta_eff=beta_eff)
        @test S2 ≈ ℓ * π * c / (12 * beta_eff) atol=1e-12

        # With boundary state / log g correction
        # For XY with boundary_state = :neumann (free), K = 1.0:
        # log g = 0.25 * log(1/2) = -0.25 * log(2)
        log_g = 0.25 * log(0.5)
        S1_bdy = QAtlas.fetch(model, VonNeumannEntropy(:quench), OBC(); ℓ=ℓ, t=t1, v=v, beta_eff=beta_eff, boundary_state=:neumann, K=1.0)
        @test S1_bdy ≈ S1 + log_g atol=1e-12

        S1_direct = QAtlas.fetch(model, VonNeumannEntropy(:quench), OBC(); ℓ=ℓ, t=t1, v=v, beta_eff=beta_eff, log_g=0.5)
    ...
    @testset "PBC global quench (revivals)" begin
        ...
        S_t1 = QAtlas.fetch(model, VonNeumannEntropy(:quench), PBC(); ℓ=ℓ, L=L, t=1.0, v=v, beta_eff=beta_eff)
        @test S_t1 ≈ 2.0 * coeff atol=1e-12

        S_t5 = QAtlas.fetch(model, VonNeumannEntropy(:quench), PBC(); ℓ=ℓ, L=L, t=5.0, v=v, beta_eff=beta_eff)
        @test S_t5 ≈ 6.0 * coeff atol=1e-12

        S_t9 = QAtlas.fetch(model, VonNeumannEntropy(:quench), PBC(); ℓ=ℓ, L=L, t=9.0, v=v, beta_eff=beta_eff)
        @test S_t9 ≈ (20.0 - 18.0) * coeff atol=1e-12

        # Check periodicity by adding L/v = 20.0 to times
        S_t21 = QAtlas.fetch(model, VonNeumannEntropy(:quench), PBC(); ℓ=ℓ, L=L, t=21.0, v=v, beta_eff=beta_eff)
        @test S_t21 ≈ S_t1 atol=1e-12

        S_t49 = QAtlas.fetch(model, VonNeumannEntropy(:quench), PBC(); ℓ=ℓ, L=L, t=49.0, v=v, beta_eff=beta_eff)
    ...
    @testset "BoundaryEntropy values" begin
        # Ising c = 1/2
        @test QAtlas.fetch(Universality(:Ising), BoundaryEntropy(), Infinite(); boundary_state=:free) ≈ 0.0 atol=1e-12
        @test QAtlas.fetch(Universality(:Ising), BoundaryEntropy(), Infinite(); boundary_state=:fixed_up) ≈ -log(2) / 2 atol=1e-12

        # XY c = 1
        # log g_N = 0.25 * log(K/2), log g_D = -0.25 * log(2K)
        @test QAtlas.fetch(Universality(:XY), BoundaryEntropy(), Infinite(); boundary_state=:neumann, K=1.0) ≈ 0.25 * log(0.5) atol=1e-12
        @test QAtlas.fetch(Universality(:XY), BoundaryEntropy(), Infinite(); boundary_state=:dirichlet, K=1.0) ≈ -0.25 * log(2.0) atol=1e-12
    ...
    @testset "OBC equilibrium entanglement boundary corrections" begin
        ...
        log_g = -log(2) / 2
        S_vn_fixed = QAtlas.fetch(model, VonNeumannEntropy(), OBC(); ℓ=ℓ, L=L, boundary_state=:fixed_up)
        @test S_vn_fixed ≈ S_vn_base + log_g atol=1e-12

        S_ren_fixed = QAtlas.fetch(model, RenyiEntropy(2.0), OBC(); ℓ=ℓ, L=L, boundary_state=:fixed_up)
