...
function render_models_index()
    ...
    function model_row(m; show_refs=false)
        hs = sort(filter(h -> modelof(h) == m, claimed))
        ...
        row =
            ...
            " |"
        if show_refs
            refs_text = _model_refs_text(m)
            row *= " " * refs_text * " |"
        end
        P(row)
    end

    function section(label, ms; show_refs=false)
        isempty(ms) && return nothing
        P("## ", label, " (", length(ms), ")")
        P("")
        if show_refs
            P("| Model | Quantities | Assurance | Universality | Key Refs |")
            P("|-------|-----------|-----------|--------------|----------|")
        else
            P("| Model | Quantities | Assurance | Universality |")
            P("|-------|-----------|-----------|--------------|")
        end
        foreach(m -> model_row(m; show_refs=show_refs), ms)
        P("")
    end
    ...
    P("")
    P("The **Universality** column shows the class detected from `CriticalExponents` /")
    P(
        "`CentralCharge` claims. `—` means no such claim is registered (not necessarily unknown).",
    )
    P("")
    section("Classical Models", classical_ms; show_refs=true)
    section("Quantum Models", quantum_ms)
    section("Other", other_ms)
    P(
        "[Full searchable catalog →](../atlas/ModelList.md) · ",
        "[Faceted search →](../atlas/by/index.md)",
    )
    P("")
    P("<!-- ATLAS:INDEX:END -->")
    return String(take!(io))
