...
@testset "SSH - Finite-T thermodynamics limits and consistency" begin
    ...
    m = SSH(; v=0.6, w=1.0)
    beta_high = 1e-5
    @test isapprox(fetch(m, ThermalEntropy(), Infinite(); beta=beta_high), log(2); atol=1e-4)
    @test isapprox(fetch(m, SpecificHeat(), Infinite(); beta=beta_high), 0.0; atol=1e-4)
    @test isapprox(fetch(m, FreeEnergy(), Infinite(); beta=beta_high), -log(2)/beta_high; atol=1e-2)

    # Low-T limits: entropy -> 0, specific heat -> 0, free energy -> E_GS
    beta_low = 100.0
    egs = fetch(m, Energy(:per_site), Infinite())
    @test isapprox(fetch(m, ThermalEntropy(), Infinite(); beta=beta_low), 0.0; atol=1e-3)
    ...
    for beta_val in [0.2, 0.5, 1.5]
        ...
        # u = d(beta * f) / dbeta
        df = ForwardDiff.derivative(b -> b * fetch(m, FreeEnergy(), Infinite(); beta=b), beta_val)
        @test isapprox(u, df; rtol=1e-5, atol=1e-7)

        # cv = -beta^2 * du/dbeta
        cv = fetch(m, SpecificHeat(), Infinite(); beta=beta_val)
        du = ForwardDiff.derivative(b -> begin
