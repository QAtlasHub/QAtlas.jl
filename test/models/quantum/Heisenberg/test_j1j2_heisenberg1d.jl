...
@testset "J1J2Heisenberg1D — j = 0 (pure Heisenberg, Bethe-Hulthén) (Phase 1)" begin
    e0 = QAtlas.fetch(
        J1J2Heisenberg1D(; J1=1.0, J2=0.0), Energy{:per_site}(), Infinite()
    )
    @test e0 ≈ 0.25 - log(2)
    # Linear scaling with J₁
    e0_3 = QAtlas.fetch(
        J1J2Heisenberg1D(; J1=3.0, J2=0.0), Energy{:per_site}(), Infinite()
    )
...
@testset "J1J2Heisenberg1D — j = 1/2 (Majumdar-Ghosh) (Phase 1)" begin
    ...
    e0_2 = QAtlas.fetch(
        J1J2Heisenberg1D(; J1=2.0, J2=1.0), Energy{:per_site}(), Infinite()
    )
    @test e0_2 ≈ -3 / 4
