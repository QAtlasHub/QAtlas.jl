...
@testset "HaldaneShastry LR velocity + slope wrappers" begin
    @testset "LiebRobinsonVelocity@Infinite" begin
        ...
        @test QAtlas.fetch(HaldaneShastry(), QAtlas.LiebRobinsonVelocity(), QAtlas.Infinite(); J=-2.0) ≈ π
    end

    @testset "EntanglementGrowthSlope@Infinite" begin
        m = HaldaneShastry()
        for β in (1.0, 2.0, 5.0, 10.0)
            slope = QAtlas.fetch(m, QAtlas.EntanglementGrowthSlope(), QAtlas.Infinite(); beta_eff=β)
            @test slope ≈ π^2 / (6 * β) atol = 1e-12
        end
        m2 = HaldaneShastry(; J=2.0)
        slope2 = QAtlas.fetch(m2, QAtlas.EntanglementGrowthSlope(), QAtlas.Infinite(); beta_eff=4.0)
