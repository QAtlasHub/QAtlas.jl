# ─────────────────────────────────────────────────────────────────────────────
# test/models/quantum/TFIM/test_TFIM_structure_factor_paramagnet_zz_batch4.jl
#
# Part 4/4 of the ZZ-axis paramagnet structure-factor batch (one (J, h) pair
# per file so the dense OBC(12) thermal ED balances finely across CI shards).
# Same paramagnet physics: at T → ∞, ρ = I / 2^N ⇒ ⟨σ^zz_i σ^zz_j⟩ = δ_ij and
# S_zzzz(q) = 1 for all q, all BC.
#
...
@testset "TFIM — ZZStructureFactor at T→∞ paramagnet = 1 (#381 batch, part 4/4)" begin
    HIGH_T_BETA = 1e-3
