...
```julia
verify(TFIM(; J = J, h = h), ThermalEntropy(), OBC(; N = N); route = :limiting_case, independent = log(2), agree_within = 1.0e-5, refs = ["TFIM OBC at T → ∞: s → log 2 per site"], fetch_kw = (; beta = HIGH_T_BETA))
```
