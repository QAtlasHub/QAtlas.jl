...
Every distinct citation string appearing in any `@register(..., references=[...], ...)` across the atlas (168 unique citations).  For each one we list which hubs cite it — a *load-bearing-ness* view: high-count entries are central, low-count entries are local.
